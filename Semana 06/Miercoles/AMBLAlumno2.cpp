...
bool archivoAlumno::sobreEscribirRegistro(Alumno reg, int pos){
    ...
    //yo encontré con buscarRegistro() el registro en el 5° registro.
    //8 x 4 = 32 bytes
...
//    }
    ...
    ///cambiar legajo a Pedro P
    pos2=reg.buscarLegajo(0);
    ...
    if(pos2>=0){
        ...
        if(reg.sobreEscribirRegistro(obj,pos2)){
            cout<<endl<<"Registro modificado"<<endl;
        }
        else{
    ...
    return 0;
    ///DAR DE BAJA A FLORENCIA PAZ LEGAJO 6666
    int auxLegajo;

    cout<<endl<<"Ingrese el legajo del alumno a dar de baja: "<<endl;
    cin>>auxLegajo; //ingresar el de Florencia.


    if(reg.bajaLogicaRegistro(auxLegajo)){
